...
public class MyImageView extends ImageView
{
	...
	private Matrix g_matrix = new Matrix();
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom)
	{
		...
		//	  Log.i("nick","view mWidth:"+mWidth+" mHeight:"+mHeight+" bitmap w:"+bitmap.getWidth()+" h"+bitmap.getHeight());
		//	bd.setAntiAlias(true);
			  //缩放不成功？？？
//			  if(mHeight<bitmap.getHeight()){
//				  
//				 float s = (float) Math.sqrt(Math.sqrt((float)mHeight/bitmap.getHeight()));
//				  beginScale(g_matrix,1,(float)mHeight/bitmap.getHeight());  
//				  beginScale(g_matrix,1,(float)mHeight/bitmap.getHeight());  
//				  beginScale(g_matrix,1,(float)mHeight/bitmap.getHeight());  
//				  Log.i("nick","-----"+(float)mHeight/bitmap.getHeight());
//			  }
		}
	}
	 ...
	 * @param scale
	 */
	private synchronized void beginScale(Matrix matrix, float w_scale,float h_scale)
	{
		matrix.postScale(w_scale, h_scale, mCenterWidth, mCenterHeight);
		setImageMatrix(matrix);
	}
